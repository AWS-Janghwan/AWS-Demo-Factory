...
export default backendUploadService;
    // 대체 엔드포인트로 업로드 시도하는 함수
    async uploadFileWithFallback(file, contentId, onProgress = () => {}) {
        // 대체 엔드포인트 목록
        ...
        for (let i = 0; i < uploadEndpoints.length; i++) {
            ...
            try {
                console.log(`🔄 [BackendUpload] 업로드 시도 ${i + 1}/${uploadEndpoints.length}: ${endpoint}`);
                ...
                const result = await new Promise((resolve, reject) => {
                    ...
                    xhr.upload.addEventListener('progress', (event) => {
                        if (event.lengthComputable) {
                            ...
                            console.log(`📊 [BackendUpload] 업로드 진행률: ${percentComplete}%`);
                    ...
                    // 요청 전송
                    const apiUrl = getBackendUrl();
                    const uploadUrl = `${apiUrl}${endpoint}`;
                    console.log(`🔥🔥 [BackendUpload] 업로드 URL: ${uploadUrl}`);
                    xhr.open('POST', uploadUrl);
                    xhr.send(formData);
                });
                ...
                console.warn(`⚠️ [BackendUpload] 엔드포인트 ${endpoint} 실패:`, error.message);
                lastError = error;
                
                // 마지막 엔드포인트가 아니면 계속 시도
                if (i < uploadEndpoints.length - 1) {
                    console.log(`🔄 [BackendUpload] 다음 엔드포인트로 재시도...`);
                    continue;
                }
            }
        }
        ...
        // 모든 엔드포인트 실패
        throw new Error(`모든 업로드 엔드포인트 실패. 마지막 오류: ${lastError?.message}`);
    }
// 클래스와 함수 모두 export
export { BackendUploadService };