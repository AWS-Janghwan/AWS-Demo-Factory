...
const initializeS3 = async () => {
  ...
  try {
    console.log("🔧 [SecureS3] AWS S3 초기화 중...");

    // 로컬 credentials 로드
    const credentials = await getLocalCredentials();

    if (!credentials) {
      throw new Error("로컬 AWS credentials를 찾을 수 없습니다");
    }
    ...
    s3Config = {
      ...
      sessionToken: credentials.sessionToken, // STS 토큰이 있는 경우
    };

    // S3 인스턴스 생성
    s3Instance = new AWS.S3(s3Config);

    console.log("✅ [SecureS3] AWS S3 초기화 완료:", {
      region: s3Config.region,
      hasCredentials: !!s3Config.accessKeyId,
    });

    return s3Instance;
  } catch (error) {
    console.error("❌ [SecureS3] S3 초기화 실패:", error);
    throw error;
  }
};
const BUCKET_NAME =
  process.env.REACT_APP_S3_BUCKET || "demo-factory-storage-bucket";
...
export const uploadFileSecurely = async (
  ...
  try {
    ...
    // S3 인스턴스 초기화
    const s3Instance = await initializeS3();
...
export const generateSecureDownloadUrl = async (s3Key, expiresIn = 86400) => {
  // 24시간 = 86400초
  try {
    await initializeS3(); // S3 인스턴스 초기화
    ...
    const presignedUrl = await s3Instance.getSignedUrlPromise(
      "getObject",
      params
    );
...
export const deleteContentFiles = async (contentId) => {
  try {
    ...
    const s3Instance = await initializeS3();
    const objects = await s3Instance.listObjectsV2(listParams).promise();
...
export const checkBucketSecurity = async () => {
  try {
    ...
    try {
      const s3Instance = await initializeS3();
      const policyResult = await s3Instance
        .getBucketPolicy({ Bucket: BUCKET_NAME })
        .promise();
    ...
    // 퍼블릭 액세스 차단 설정 확인
    try {
      const blockResult = await s3Instance
        .getPublicAccessBlock({ Bucket: BUCKET_NAME })
        .promise();
      console.log(
        "🔒 [SecureS3] 퍼블릭 액세스 차단 설정:",
        blockResult.PublicAccessBlockConfiguration
      );

      const isSecure =
        blockResult.PublicAccessBlockConfiguration.BlockPublicAcls &&
